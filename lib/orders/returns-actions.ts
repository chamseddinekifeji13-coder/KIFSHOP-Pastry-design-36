...
export async function fetchReturns(tenantId: string): Promise<OrderReturn[]> {
  ...
  try {
    // 1. Fetch from order_returns table without using the problematic inner join
    const { data, error } = await supabase
      .from("order_returns")
      .select(`
        id, tenant_id, order_id, return_type, reason, reason_details,
        status, refund_method, refund_amount, notes, created_at
      `)
    ...
    if (error) {
      ...
      // Table might not exist - fall through to best_delivery returns
    } else if (data && data.length > 0) {
      // For each return, fetch items and related order data separately
      const returns: OrderReturn[] = []
      for (const r of data) {
        // Fetch order details separately to avoid join issues
      ...
      // Also add returns from best_delivery_shipments
    ...
    console.error("Exception fetching returns:", err.message)
  }

  // If order_returns table doesn't exist, just return best_delivery returns
  const { data: bdReturns } = await supabase
    .from("best_delivery_shipments")
    .select("id, order_id, customer_name, customer_phone, cod_amount, tracking_number, created_at, notes")
    .eq("tenant_id", tenantId)
...
export async function createReturn(data: CreateReturnData): Promise<boolean> {
  ...
  try {
    ...
    // Try to record in status history if table exists
    try {
      await supabase.from("order_status_history").insert({
        ...
        note: `Retour ${data.returnType === "total" ? "total" : "partiel"} - ${reasonLabels[data.reason]}`,
      })
    } catch (histErr: any) {
      // Silently fail if order_status_history table doesn't exist
      console.debug("Could not record status history:", histErr.message)
    }

    return true
  } catch (err: any) {
    console.error("Error in createReturn:", err.message)
    return false
  }
}
...
export async function processReturn(
  ...
  try {
    // Get the return (without using problematic join)
    ...
    const { data: order } = await supabase
      .from("orders")
      .select("customer_name, customer_phone, total, deposit")
    ...
    if (action === "completed" && returnRecord.refund_method === "credit_note") {
      try {
        ...
        // Link credit to return if it was created
        if (credit) {
          await supabase
            .from("order_returns")
        ...
        console.error("Error creating customer credit:", creditErr.message)
        // Don't fail the return processing if credit creation fails
      }
    }
    ...
    console.error("Error in processReturn:", err.message)
    return false
  }
}
