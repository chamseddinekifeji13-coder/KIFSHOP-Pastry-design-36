...
function ProductionWorkflowIllustration() {
  return (
    <div className="rounded-2xl border border-border/50 bg-card overflow-hidden shadow-lg shadow-primary/10 hover:shadow-xl hover:shadow-primary/20 transition-all duration-300">
      {/* Header */}
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-2">
          {["Recette", "Production", "Controle", "Livraison"].map((step, i) => (
            <div key={step} className="flex items-center gap-2 flex-1">
              <div className={`flex items-center justify-center rounded-full h-6 w-6 text-[9px] font-bold text-white ${i <= 1 ? "bg-primary" : "bg-muted-foreground/30"}`}>
                {i < 2 ? (
              ...
              {i < 3 && <div className={`flex-1 h-px ${i < 1 ? "bg-primary" : "bg-border"}`} />}
            </div>
          ))}
        </div>
        ...
        <div className="rounded-lg border border-border/50 bg-muted/20 p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[9px] font-medium text-primary">En production</span>
          </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={product.name} className="rounded-lg border border-border/50 bg-muted/20 p-2 text-center">
              <div className="mx-auto mb-1.5 flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10">
        ...
        <div className="flex items-center gap-2">
          ...
          {[
            ...
            { name: "Instagram", color: "bg-destructive/10 text-destructive" },
            { name: "Telephone", color: "bg-primary/10 text-primary" },
          ].map((ch) => (
            ...
            {[
              ...
              <div key={order.client} className="flex items-center justify-between rounded-md bg-background px-2.5 py-1.5">
                ...
                <div className="text-right">
                  ...
                  <p className={`text-[8px] ${order.status === "Confirmee" ? "text-primary" : order.status === "Livree" ? "text-success" : "text-warning"}`}>
                    {order.status}
                  </p>
                </div>
              </div>
