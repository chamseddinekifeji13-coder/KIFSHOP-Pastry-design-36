...
export function NewOrderDrawer({ open, onOpenChange }: NewOrderDrawerProps) {
  ...
  const [source, setSource] = useState<"comptoir" | "whatsapp" | "messenger" | "phone" | "web" | "instagram">("comptoir")
                ...
                <SelectContent>
                  {products.map(product => (
                    <SelectItem key={product.id} value={product.id}>
                      {product.name} - {product.price.toLocaleString("fr-TN")} TND
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                  ...
                  <div key={item.productId} className="flex items-center justify-between p-3">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {item.price.toLocaleString("fr-TN")} TND / unité
                      </p>
                    </div>
